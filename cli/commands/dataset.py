...
def build_dataset(author_id: str = typer.Argument(..., help="Author ID to build dataset for")):
    ...
    if not profile:
        ...
        console.print("Use 'ghostwriter author create' to create an author first.")
        raise typer.Exit(1)
...
def validate_dataset(author_id: str = typer.Argument(..., help="Author ID to validate dataset for")):
    ...
    if not dataset or dataset.size == 0:
        ...
        console.print(f"Use 'ghostwriter dataset build {author_id}' to create one.")
        raise typer.Exit(1)
...
def show_dataset(author_id: str = typer.Argument(..., help="Author ID to show dataset for")):
    ...
    console.print(f"[dim]Created: {dataset.created_at.strftime('%Y-%m-%d %H:%M')}[/dim]")
    console.print(f"[dim]Updated: {dataset.updated_at.strftime('%Y-%m-%d %H:%M')}[/dim]")
    ...
    if dataset.size > 0:
        ...
        for i, example in enumerate(dataset.examples[:3], 1):
            ...
            console.print(f"[dim]Prompt:[/dim] {user_msg[:100]}{'...' if len(user_msg) > 100 else ''}")
            console.print(f"[dim]Response:[/dim] {assistant_msg[:200]}{'...' if len(assistant_msg) > 200 else ''}")
...
def clear_dataset(author_id: str = typer.Argument(..., help="Author ID to clear dataset for")):
    ...
    console.print(f"[red]⚠️  This will delete all {dataset.size} training examples for '{profile.name}'[/red]")
...
def export_dataset(
    ...
    output_file: str = typer.Option(None, "--output", "-o", help="Output file path (default: <author_id>_dataset.jsonl)")
):
    ...
    try:
        ...
        console.print(f"[green]✅ Dataset exported to '{output_file}' ({dataset.size} examples)[/green]")
