...
def start_training(
    ...
    if not dataset or dataset.size == 0:
        ...
        console.print(
            f"Use 'ghostwriter dataset build {author_id}' to create one first."
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print(
            f"• Check status: [cyan]ghostwriter train status {author_id}[/cyan]"
        )
        console.print(
            f"• List all jobs: [cyan]ghostwriter train list {author_id}[/cyan]"
        )
