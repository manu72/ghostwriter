...
from core.storage import get_author_profile, AuthorStorage
...
def start_training(
    ...
    base_model: str = typer.Option("gpt-3.5-turbo", "--model", "-m", help="Base model to fine-tune"),
    wait: bool = typer.Option(False, "--wait", "-w", help="Wait for training to complete")
):
    ...
    if not dataset or dataset.size == 0:
        ...
        console.print(f"Use 'ghostwriter dataset build {author_id}' to create one first.")
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print(f"• Check status: [cyan]ghostwriter train status {author_id}[/cyan]")
        console.print(f"• List all jobs: [cyan]ghostwriter train list {author_id}[/cyan]")
...
def check_status(author_id: str = typer.Argument(..., help="Author ID to check training status for")):
    ...
    if not metadata.fine_tune_jobs:
        ...
        console.print(f"Use 'ghostwriter train start {author_id}' to start training.")
        return
    ...
    try:
        ...
        for job in sorted(metadata.fine_tune_jobs, key=lambda x: x.created_at, reverse=True):
            status_color = {
            ...
            status_display = f"[{status_color}]{job.status.value.upper()}[/{status_color}]"
            
            table.add_row(
                job.job_id[:17] + "..." if len(job.job_id) > 20 else job.job_id,
                status_display,
                job.base_model,
                job.fine_tuned_model[:27] + "..." if job.fine_tuned_model and len(job.fine_tuned_model) > 30 else (job.fine_tuned_model or "-"),
...
def test_model(
    ...
    model_id: str = typer.Option(None, "--model", "-m", help="Specific model ID to test (default: latest successful)")
):
    ...
    # Find model to test
    if model_id:
        job = next((job for job in metadata.fine_tune_jobs if job.fine_tuned_model == model_id), None)
        if not job:
    ...
    try:
        ...
        results = adapter.test_fine_tuned_model(model_id, test_prompts)
...
def generate_text(
    ...
    prompt: str = typer.Option(None, "--prompt", "-p", help="Prompt for text generation"),
    model_id: str = typer.Option(None, "--model", "-m", help="Specific model ID (default: latest successful)"),
    max_tokens: int = typer.Option(500, "--max-tokens", help="Maximum tokens to generate")
):
    ...
    try:
        ...
        console.print(Panel(
            response,
            title=f"✍️  Generated by {profile.name}",
            border_style="green"
        ))
