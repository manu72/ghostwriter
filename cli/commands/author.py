...
def create_author(
    ...
    interactive: bool = typer.Option(True, "--interactive/--no-interactive", "-i/-ni", help="Interactive setup")
):
...
def list_authors_cmd():
    ...
    if not authors:
        console.print("[yellow]No authors found. Run 'ghostwriter author create' to create one.[/yellow]")
        return
    ...
    for author_id in authors:
        ...
        created_str = profile.created_at.strftime("%Y-%m-%d") if hasattr(profile, 'created_at') else "Unknown"
        
        table.add_row(
            author_id,
            profile.name,
            profile.description[:47] + "..." if len(profile.description) > 50 else profile.description,
            style_summary,
            created_str
        )
...
def show_author(author_id: str = typer.Argument(..., help="Author ID to display")):
    ...
    if metadata and metadata.fine_tune_jobs:
        ...
        if latest_job and latest_job.fine_tuned_model:
            console.print(f"[green]✅ Active model: {latest_job.fine_tuned_model}[/green]")
    else:
...
def edit_author(author_id: str = typer.Argument(..., help="Author ID to edit")):
    ...
    style.tone = Prompt.ask("Tone", default=style.tone, 
                           choices=["casual", "professional", "friendly", "authoritative", "witty", "formal"])
    
    style.voice = Prompt.ask("Voice", default=style.voice,
                            choices=["first_person", "second_person", "third_person"])
    
    style.formality = Prompt.ask("Formality", default=style.formality,
                                choices=["very_casual", "casual", "moderate", "formal", "academic"])
    
    style.length_preference = Prompt.ask("Length Preference", default=style.length_preference,
    ...
    style.writing_style_notes = Prompt.ask("Additional style notes", default=style.writing_style_notes)
...
def create_author_interactive(author_id: str = None, name: str = None, description: str = None):
    ...
    # Get basic info
    if not author_id:
        author_id = Prompt.ask("Enter a unique ID for this author (e.g., 'john_doe', 'blog_writer')")
    ...
    if not name:
        name = Prompt.ask("What should we call this author?", default=author_id.replace("_", " ").title())
    
    if not description:
        description = Prompt.ask("Brief description of this author (optional)", default="")
    ...
    topics_str = Prompt.ask("What topics do you typically write about? (comma-separated, optional)", default="")
    topics = [t.strip() for t in topics_str.split(",") if t.strip()]
    
    avoid_str = Prompt.ask("Any topics you want to avoid? (comma-separated, optional)", default="")
    avoid_topics = [t.strip() for t in avoid_str.split(",") if t.strip()]
    
    style_notes = Prompt.ask("Any additional notes about your writing style? (optional)", default="")
    ...
    console.print(Panel(
        ...
        f"   [cyan]ghostwriter dataset build {author_id}[/cyan]\n\n"
...
def create_author_simple(author_id: str, name: str = None, description: str = ""):
    ...
    console.print(f"[dim]Use 'ghostwriter author edit {author_id}' to customize the style.[/dim]")