...
class TestChatSessionCommand:
    ...
    def test_chat_session_no_fine_tuned_model(self, mock_storage_class, mock_get_profile, mock_print):
        ...
        assert exc_info.value.exit_code == 1
        mock_print.assert_any_call("[red]No fine-tuned model found for 'test_author'.[/red]")
    ...
    def test_chat_session_resume_nonexistent_session(
        ...
        assert exc_info.value.exit_code == 1
        mock_print.assert_any_call("[red]Chat session 'nonexistent-session' not found.[/red]")
...
class TestChatDisplayFunctions:
    ...
    def test_display_chat_header(self):
        ...
        with patch("cli.commands.generate.console.print") as mock_print:
            _display_chat_header("Test Author", "ft:gpt-3.5-turbo:model:123", session, True)
    ...
    def test_display_conversation_history(self):
        ...
        with patch("cli.commands.generate._display_user_message") as mock_display_user, \
             patch("cli.commands.generate._display_assistant_message") as mock_display_assistant, \
             patch("cli.commands.generate.console.print") as mock_print:
    ...
    def test_display_assistant_message(self):
        ...
        with patch("cli.commands.generate.console.print") as mock_print:
            _display_assistant_message("Hello there!", "Test Author", show_timestamp=False)
...
class TestChatCommandHandlers:
    ...
    def test_handle_history_command_with_messages(self):
        ...
        with patch("cli.commands.generate._display_conversation_history") as mock_display:
    ...
    def test_handle_save_command(self):
        ...
        with patch("cli.commands.generate.console.print") as mock_print:
            ...
            assert result is True
            storage.save_chat_session.assert_called_once_with(session)
            mock_print.assert_any_call(f"[green]Session saved: {session.session_id}[/green]")
    ...
    def test_handle_export_command(self):
        ...
        with patch("cli.commands.generate.console.print") as mock_print:
            for export_cmd in ["/export", "/export-md"]:
                result = _handle_chat_command(export_cmd, session, storage, "Test Author")
                assert result is True
                
            storage.export_chat_session_as_markdown.assert_called_with(session, "Test Author")
            mock_print.assert_any_call("[green]Chat exported to: chat_export.md[/green]")
