...
from core.models import AuthorProfile, StyleGuide, Dataset
...
class TestCLIBasicCommands:
    ...
    def test_version_command(self):
        ...
        assert "Ghostwriter" in result.stdout
    ...
    def test_status_command_no_authors(self, mock_list_authors):
        ...
        assert "ghostwriter init" in result.stdout
    
    @patch('core.storage.list_authors')
    @patch('core.storage.get_author_profile')
    @patch('core.storage.AuthorStorage')
    def test_status_command_with_authors(self, mock_storage_class, mock_get_profile, mock_list_authors, sample_author_profile):
        ...
        mock_storage.load_model_metadata.return_value = Mock(
            fine_tune_jobs=[],
            get_latest_successful_job=Mock(return_value=None)
        )
...
class TestAuthorCommands:
    ...
    def test_author_create_interactive(self, mock_confirm, mock_prompt, mock_storage_class, temp_data_dir):
    ...
    def test_author_list_with_authors(self, mock_get_profile, mock_list_authors, sample_author_profile):
    ...
    def test_author_create_non_interactive(self):
        ...
        with patch('cli.commands.author.create_author_simple') as mock_create:
            ...
            mock_create.assert_called_once_with("test_author", "Test Author", "Test description")
...
class TestInitCommand:
    ...
    def test_init_command(self, mock_create_author):
        ...
        assert result.exit_code == 0
        assert "Welcome to Ghostwriter!" in result.stdout
...
class TestFullWorkflow:
    ...
    def test_complete_author_workflow(self, mock_storage_settings, mock_adapter_settings, temp_data_dir):
    ...
    def test_command_help_messages(self):
        ...
        for cmd in commands_to_test:
            ...
            assert result.exit_code == 0
            assert "--help" in result.stdout or "Usage:" in result.stdout or "help" in result.stdout.lower()
...
class TestCLIPerformance:
    ...
    def test_status_command_performance(self):
        ...
        with patch('core.storage.list_authors') as mock_list, \
             patch('core.storage.get_author_profile') as mock_profile, \
             patch('core.storage.AuthorStorage') as mock_storage:
