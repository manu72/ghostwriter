...
class StyleGuide(BaseModel):
    tone: str = Field(default="professional", description="Writing tone (e.g., casual, professional, witty)")
    voice: str = Field(default="first_person", description="Narrative voice (e.g., first_person, third_person)")
    formality: str = Field(default="moderate", description="Formality level (e.g., casual, moderate, formal)")
    length_preference: str = Field(default="medium", description="Preferred content length (e.g., short, medium, long)")
    topics: List[str] = Field(default_factory=list, description="Preferred topics or domains")
    avoid_topics: List[str] = Field(default_factory=list, description="Topics to avoid")
    writing_style_notes: str = Field(default="", description="Additional style preferences")
...
class FineTuneJob(BaseModel):
    ...
    created_at: datetime = Field(default_factory=datetime.now)
    updated_at: datetime = Field(default_factory=datetime.now)
    training_file_id: Optional[str] = Field(None, description="Provider's training file ID")
    fine_tuned_model: Optional[str] = Field(None, description="ID of the fine-tuned model")
...
class ModelMetadata(BaseModel):
    ...
    active_model: Optional[str] = Field(None, description="Currently active fine-tuned model")
    ...
    def get_latest_successful_job(self) -> Optional[FineTuneJob]:
        successful_jobs = [job for job in self.fine_tune_jobs if job.status == JobStatus.SUCCEEDED]
        return max(successful_jobs, key=lambda x: x.updated_at) if successful_jobs else None