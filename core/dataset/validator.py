...
class DatasetValidator:
    ...
    def _check_size(self) -> Dict[str, Any]:
        ...
        if size == 0:
            ...
            recommendation = "Add more examples (recommended: 10-100) for better fine-tuning"
        elif size < 100:
            status = "good"
    ...
    def _check_format(self) -> Dict[str, Any]:
        ...
        elif len(issues) < len(self.dataset.examples) * 0.1:  # Less than 10% have issues
            return {
                "status": "warning",
    ...
    def _check_content_quality(self) -> Dict[str, Any]:
        ...
        if stats["too_short_responses"] > len(self.dataset.examples) * 0.3:
            issues.append(f"{stats['too_short_responses']} very short responses (< 10 words)")
        if stats["too_long_responses"] > 0:
            issues.append(f"{stats['too_long_responses']} very long responses (> 1000 words)")
        ...
        return {
            "status": status,
            "message": message,
            "recommendation": "Review and improve short or empty responses" if issues else "Content quality is acceptable",
            "stats": stats
        }
    ...
    def _check_diversity(self) -> Dict[str, Any]:
        ...
        for i in range(len(response_words)):
            for j in range(i+1, min(i+10, len(response_words))):  # Check against next 10 responses
                if len(response_words[i]) > 0 and len(response_words[j]) > 0:
                    overlap = len(response_words[i] & response_words[j]) / len(response_words[i] | response_words[j])
        ...
        return {
            "status": status,
            "message": message,
            "recommendation": "Add more varied prompts and responses" if issues else "Diversity looks good"
        }
    
    def _check_safety(self) -> Dict[str, Any]:
        # Basic safety checks for problematic content
        safety_keywords = [
            "violence", "violent", "kill", "murder", "death", "suicide", "harm", "hurt",
            "hate", "racist", "sexist", "discrimination", "abuse", "illegal", "drug",
            "weapon", "bomb", "terror", "steal", "fraud", "scam"
        ]
