...
from core.models import AuthorProfile, Dataset, TrainingExample, StyleGuide
from core.storage import AuthorStorage
from core.prompts.templates import DATASET_BUILDING_PROMPTS, EXAMPLE_GENERATION_PROMPTS, get_example_prompt
...
class DatasetBuilder:
    ...
    def _add_from_writing_samples(self):
        console.print(Panel(DATASET_BUILDING_PROMPTS["writing_sample"], title="Writing Sample"))
        ...
        console.print(Panel(DATASET_BUILDING_PROMPTS["prompt_for_sample"], title="Prompt Creation"))
        ...
        console.print(f"[green]Added example! Dataset now has {self.dataset.size} examples[/green]")
    ...
    def _generate_examples(self):
        ...
        choice = Prompt.ask("Select prompt type", choices=[str(i) for i in range(1, len(prompt_types) + 1)])
        ...
        user_prompt = prompt_template.format(topic=topic, tone="professional", length_preference="medium")
    ...
    def _import_from_file(self):
        ...
        try:
            ...
            console.print(f"[green]Import complete! Dataset now has {self.dataset.size} examples[/green]")
    ...
    def _review_dataset(self):
        ...
        for i, example in enumerate(self.dataset.examples[:10], 1):  # Show first 10
            ...
            response_preview = assistant_msg[:47] + "..." if len(assistant_msg) > 50 else assistant_msg
    ...
    def _save_and_exit(self):
        ...
        console.print(f"[green]Saved {self.dataset.size} examples to {self.storage.author_dir / 'train.jsonl'}[/green]")
        
        if self.dataset.size < 10:
            console.print("[yellow]Recommendation: Add more examples (10-100) for better fine-tuning results[/yellow]")
        elif self.dataset.size >= 100:
            console.print("[green]Great! You have enough examples for effective fine-tuning[/green]")