...
class HistoricalProfileGenerator:
    ...
    def generate_profile(
        ...
        """
        console.print(
            f"[blue]🎭 Generating author profile for {figure_analysis.figure_name}...[/blue]"
        )
        ...
        console.print(
            f"[green]✅ Generated profile for {figure_analysis.figure_name}[/green]"
        )
        return profile

    def generate_style_guide(
        self, figure_analysis: FigureAnalysis
    ) -> Optional[StyleGuide]:
        ...
        prompt = STYLE_GUIDE_GENERATION_TEMPLATE.format(
            figure_name=figure_analysis.figure_name, figure_analysis=full_analysis
        )
...
{analysis.historical_context}
    ...
    def _generate_description(self, analysis: FigureAnalysis) -> str:
        ...
        return (
            f"AI author based on {analysis.figure_name}'s writing style and voice. "
    ...
    def _parse_style_guide(
        self, response: str, figure_name: str
    ) -> Optional[StyleGuide]:
        ...
        try:
            ...
            length_preference = (
                self._extract_style_field(response, "LENGTH_PREFERENCE") or "medium"
            )
            ...
            avoid_topics_text = (
                self._extract_style_field(response, "AVOID_TOPICS") or ""
            )
            avoid_topics = self._parse_topic_list(avoid_topics_text)

            # Extract style notes
            style_notes = (
                self._extract_style_field(response, "WRITING_STYLE_NOTES") or ""
            )

            # Validate the extracted values
            tone = self._validate_choice(
                tone,
                [
                    "casual",
                    "professional",
            ...
            voice = self._validate_choice(
                voice, ["first_person", "second_person", "third_person"], "first_person"
            )
            formality = self._validate_choice(
                formality,
                ["very_casual", "casual", "moderate", "formal", "academic"],
                "moderate",
            )
            length_preference = self._validate_choice(
                length_preference, ["short", "medium", "long", "variable"], "medium"
            )
            ...
            console.print(
                f"[green]✅ Style guide parsed successfully for {figure_name}[/green]"
            )
            return style_guide
    ...
    def _parse_topic_list(self, text: str) -> List[str]:
        ...
        topics = [
            topic
            for topic in topics
            if topic and topic.lower() not in ["none", "n/a", "not specified"]
        ]
        return topics[:10]  # Limit to 10 topics max

    def _validate_choice(
        self, value: str, valid_choices: List[str], default: str
    ) -> str:
        ...
        console.print(
            f"[yellow]⚠️  Invalid value '{value}', using default '{default}'[/yellow]"
        )
        return default
    ...
    def customize_profile(self, profile: AuthorProfile) -> AuthorProfile:
        ...
        new_topics = Prompt.ask(
            "Preferred topics (comma-separated)", default=topics_str
        )
        ...
        style.writing_style_notes = Prompt.ask(
            "Style notes", default=style.writing_style_notes
        )
