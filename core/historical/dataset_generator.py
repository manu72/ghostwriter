...
class HistoricalDatasetGenerator:
    ...
    def generate_initial_dataset(
        ...
        """
        console.print(f"[blue]📚 Generating initial dataset for {profile.name} ({count} examples)[/blue]")
        ...
        for batch_start, batch_count in batches:
            console.print(f"[dim]Generating batch {batch_start//batch_size + 1}: examples {batch_start+1}-{batch_start+batch_count}[/dim]")
            ...
            if batch_examples:
                ...
                console.print(f"[green]Batch complete: {len(approved)}/{len(batch_examples)} examples approved[/green]")
            else:
                console.print("[yellow]⚠️  Failed to generate batch, continuing with next...[/yellow]")
        
        if total_generated == 0:
            console.print("[red]❌ No examples were generated successfully[/red]")
            return None
        
        console.print(f"[green]🎉 Generated {total_generated} training examples for {profile.name}![/green]")
        return dataset
    ...
    def add_examples_to_dataset(
        ...
        """
        console.print(f"[blue]📝 Adding {count} more examples to {profile.name}'s dataset[/blue]")
    ...
    def _generate_example_batch(
        ...
        try:
            ...
            prompt = HISTORICAL_EXAMPLE_GENERATION_TEMPLATE.format(
                ...
                style_notes=profile.style_guide.writing_style_notes or "No specific notes",
    ...
    def _create_historical_context(self, analysis: FigureAnalysis) -> str:
        ...
        if analysis.unique_characteristics != "Section not found":
            context_parts.append(f"Unique Characteristics: {analysis.unique_characteristics}")
        
        return "\n\n".join(context_parts) if context_parts else "No specific historical context available."
    ...
    def _parse_generated_examples(self, response: str) -> List[TrainingExample]:
        ...
        try:
            ...
            example_sections = re.split(example_pattern, response)[1:]  # Skip first empty section
            ...
            for section in example_sections:
                ...
                assistant_match = re.search(assistant_pattern, section, re.DOTALL | re.IGNORECASE)
    ...
    def _fallback_parse_examples(self, response: str) -> List[TrainingExample]:
        ...
        for line in lines:
            ...
            elif current_response_lines is not None and line and not line.startswith(('User prompt:', 'EXAMPLE')):
    ...
    def _review_examples(self, examples: List[TrainingExample], figure_name: str) -> List[TrainingExample]:
        ...
        for i, example in enumerate(examples):
            ...
            response_preview = assistant_msg[:300] + "..." if len(assistant_msg) > 300 else assistant_msg
            ...
            if len(assistant_msg) > 300:
                if Confirm.ask("Show full response?"):
                    console.print(Panel(assistant_msg, border_style="dim", title="Full Response"))
            ...
            if action == "accept":
                ...
                new_response = Prompt.ask("Enter new response (or press Enter to keep current)", default=assistant_msg)
                ...
                if new_response.strip():
                    ...
                    console.print("[yellow]⚠️  No changes made, skipping example[/yellow]")
    ...
    def preview_dataset(self, dataset: Dataset, figure_name: str) -> None:
        ...
        for i, example in enumerate(dataset.examples[:3], 1):
            ...
            console.print(f"[dim]Prompt:[/dim] {user_msg[:80]}{'...' if len(user_msg) > 80 else ''}")
            console.print(f"[dim]Response:[/dim] {assistant_msg[:120]}{'...' if len(assistant_msg) > 120 else ''}")
    ...
    def suggest_dataset_size(self, figure_name: str) -> int:
        ...
        while True:
            try:
                size = int(Prompt.ask("How many examples would you like to generate?", default="20"))
                if 1 <= size <= 100:
                    # Show cost estimate
                    cost = self.estimate_generation_cost(size)
                    console.print(f"[yellow]💰 Estimated cost for {size} examples: ${cost:.3f}[/yellow]")
                    return size
                else:
